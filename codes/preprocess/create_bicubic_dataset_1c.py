...
if opt.dataset == 'df2k':
    ...
    path_sdsr = PATHS['datasets']['yuv_y'] + '/generated/sdsr/'
    path_tdsr = PATHS['datasets']['yuv_y'] + '/generated/tdsr/'
    input_source_dir = PATHS['yuv_y']['onec']['source']
    input_target_dir = PATHS['yuv_y']['onec']['target']
    source_files = [os.path.join(input_source_dir, x) for x in os.listdir(input_source_dir) if utils.is_image_file(x)]
    target_files = [os.path.join(input_target_dir, x) for x in os.listdir(input_target_dir) if utils.is_image_file(x)]
else:
...
with torch.no_grad():
    ...
    for file in tqdm(target_files, desc='Generating images from target'):
        ...
        # 아래는 원본 3-channel 구조 코드. 복구 시 단순히 주석 삭제로 복원 
        # resize_img = utils.imresize(input_img, 1.0 / opt.upscale_factor, True)
        
        ###########################################################################
        # 아래는 새로운 1-channel 구조 코드 (딱 하단의 코드 한 줄만 수정함. )
        resize_img = utils.imresize_1c(input_img, 1.0 / opt.upscale_factor, True)
        # 이 이하로 수정하지 않음. 
        ###########################################################################
